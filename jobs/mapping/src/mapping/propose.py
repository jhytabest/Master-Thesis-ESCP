...
def build_client() -> Any:
  if genai is None or genai_types is None:
    raise RuntimeError("google-genai is unavailable; use --model local-heuristic for offline mode")
  project = os.getenv("GCP_PROJECT_ID")
  if not project:
    raise RuntimeError("Missing GCP_PROJECT_ID; set it or use --model local-heuristic")
...
def generate_with_timeout(client: Any, model_name: str, prompt: str) -> Any:
  def _call() -> Any:
...
def default_model_name() -> str:
  ...
  if configured:
    return configured
  if os.getenv("LOCAL_STORAGE_ROOT"):
    return "local-heuristic"
  return "gemini-3-flash-preview"


def heuristic_group_output(group_name: str, contexts: List[Dict[str, Any]]) -> Dict[str, Any]:
  columns: List[Dict[str, Any]] = []
  for context in contexts:
    column = context["column"]
    detected_type = context.get("detected_type")
    distinct_values = context.get("distinct_values") or []
    normalized_column = normalize_header(column)
    action = "skip"
    value_map: List[Dict[str, Any]] = []
    list_delimiters = context.get("list_delimiters") or []

    if detected_type == "numeric_ish":
      action = "parse_numeric"
      normalized_column = default_normalized_column(column, action)
    elif detected_type == "multi_value_list":
      action = "split_list"
      normalized_column = default_normalized_column(column, action)
    elif detected_type == "categorical" and len(distinct_values) <= 200:
      action = "map_values"
      normalized_column = default_normalized_column(column, action)
      value_map = [
        {
          "raw": raw,
          "normalized": normalize_header(raw) or raw.strip().lower(),
          "confidence": 0.5
        }
    ...
    columns.append({
      ...
      "notes": "generated by local heuristic mode",
      "confidence": 0.5
    })

  return {"group_name": group_name, "columns": columns, "consolidations": []}
...
def main() -> None:
  ...
  use_local_heuristic = args.model == "local-heuristic"
  ai_client: Optional[Any] = None
  if not use_local_heuristic:
    try:
      ai_client = build_client()
    except Exception as exc:
      log(f"[model] falling back to local heuristic mode: {exc}")
      use_local_heuristic = True
  ...
  grouping_prompt_hash = ""
  if use_local_heuristic:
    groups = [{"group_name": "singleton", "columns": [col], "notes": "local heuristic grouping"} for col in columns]
    grouping_prompt_hash = sha256_text(json.dumps(columns))
    log("[grouping] local heuristic singleton groups")
  else:
  ...
  def process_group(group: Dict[str, Any]) -> None:
    ...
    output: Dict[str, Any] = {}
    if use_local_heuristic:
      output = heuristic_group_output(group_name, contexts)
      prompt_hash = sha256_text(f"local-heuristic::{group_name}")
    else:
